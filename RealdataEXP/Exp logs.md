...
针对大规模数据集（如KuaiRand-1K, 27K）处理时的内存瓶颈和性能问题，进行了一系列核心架构升级和稳定性修复。

1.  **数据分块与磁盘缓存机制**:
    * **核心功能**: 引入了数据分块（Chunking）处理机制。在首次运行时，将庞大的数据集按用户分割成多个配置数量的小数据块（Chunks）。
    * **性能优化**: 每个数据块经过特征处理后，以高效的NumPy格式缓存到磁盘。后续实验运行时，系统将直接从磁盘加载这些预处理好的数据块，极大地加快了启动和数据准备速度。
    * **内存优化**: 预训练阶段，每个epoch只加载一个数据块到内存中进行训练，从根本上解决了因一次性加载整个数据集而导致的内存不足（OOM）问题。

2.  **周期性动态数据重分块**:
    * **增加随机性**: 新增 `rechunk_every_n_epochs` 配置项，允许在训练过程中每隔N个epoch，自动重新加载原始数据、随机划分用户并生成新的数据块缓存。
    * **目的**: 此功能旨在通过周期性地改变数据批次的构成，增加模型训练的随机性，有助于提升模型的泛化能力。

3.  **数据集特定的缓存隔离**:
    * **问题修复**: 解决了不同数据集（如 `KuaiRand-1K` 和 `KuaiRand-27K`）会相互覆盖缓存文件（特别是 `scaler.pkl`, `dim_info.pkl` 等）的严重缺陷。
    * **解决方案**: 现在所有的缓存文件，包括数据块和特征处理器状态，都会自动以数据集名称作为文件名前缀（例如 `KuaiRand-1K_scaler.pkl`），确保了不同实验的缓存数据可以安全共存。

4.  **增强的检查点保存逻辑**:
    * **迭代溯源**: 所有的模型权重保存文件名现在都会自动附加当前的迭代次数（如 `pretrain_latest_iter_10000.pt`），便于精确追踪和恢复模型状态。
    * **定期备份**: 新增逻辑，每隔10000次迭代会无条件保存一次当前的模型权重，作为训练过程中的一个稳定备份。

5.  **关键Bug修复与代码重构**:
    * **修复 `inf` 损失**: 解决了 `play_time` 标签在使用 `LogMAELoss` 时因模型输出非正数而导致损失变为 `inf` 的问题。通过在计算损失前将模型输出 `clamp`到一个极小的正数（1e-6）来确保数值稳定性。
    * **修复 `Too many open files` 错误**: 移除了 `DataLoader` 中的 `persistent_workers` 参数，解决了因在循环中重复创建数据加载器而导致工作进程和文件句柄泄漏，最终使程序崩溃的问题。
    * **消除 `PerformanceWarning`**: 优化了 `FeatureProcessor` 中处理分类特征的代码，使用 `DataFrame.reindex` 替代了逐列添加的低效操作，消除了DataFrame碎片化警告。
    * **代码逻辑优化**: 将 `torch.compile` 和 `ipex.optimize` 等模型优化相关的代码从数据加载函数中剥离，移至独立的 `_apply_model_optimizations` 方法中，使得代码结构更清晰，逻辑更合理。
...
1. **可配置的高级优化**:
   ...
   * 在配置文件中新增 `optimization` 部分，允许启用或禁用 `torch.compile` 并选择后端（如 `ipex`）。
   * 将优化器和学习率调度器变为可配置项，现在可以为每个标签独立选择 `Adam` 或 `Lookahead` 优化器，以及 `ReduceLROnPlateau` 或 `CosineAnnealingLR` 等调度器。

2. **可恢复的训练与智能检查点**:

   * 通过在配置文件中设置 `load_checkpoint_path`，现在可以从指定的检查点恢复训练，同时加载模型、优化器和调度器的状态。
   * 修复了 `Lookahead` 优化器在保存和加载检查点时因其包装器结构而导致的崩溃问题。
   * 优化了检查点保存逻辑，不再为无用的 `inf_count` 指标保存模型。

3. **加权的“最佳模型”选择**:

   * 引入了 `best_overall_score` 配置项，取代了原有的 `primary_metric`。现在可以通过为多个验证指标（如 `val_play_time_loss`, `val_click_auc` 等）分配不同权重，来更灵活地定义“最佳综合模型”的标准。

4. **IPEX 兼容性与稳定性**:

   * 解决了在IPEX后端启用 `torch.compile` 时因缺少 `convolution_forward` 算子而导致的崩溃问题。
   * 新增 `ipex_strategy` 配置项，允许在 `ipex.optimize()`（稳定）和 `torch.compile`（实验性）之间为IPEX后端选择优化策略。
   * 修复了在IPEX后端使用混合精度（AMP）与非SGD优化器（如Adam, Lookahead）时可能导致驱动崩溃的稳定性问题。
